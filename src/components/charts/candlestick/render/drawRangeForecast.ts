 ...
 *  start point (Current) to independently-placed Max/Avg/Min targets — Max/Min solid, Avg
 *  dotted — with the triangular area between the Max and Min lines filled, and every point
...
export function drawRangeForecastDrawings(ctx: CanvasRenderingContext2D, params: RenderCandlestickChartParams, style: ChartCanvasStyle) {
  ...
  for (const dr of visibleDrawings) {
    if (dr.lineType !== "rangeForecast" || (dr.extraPoints?.length ?? 0) < 2) continue;
    ...
    const avg = { x: zoomedXScale(indexForDate(avgPoint.x) + 0.5), y: zoomedPriceScale(avgPoint.y) };
    ...
    const avgPill = pillFor("Avg", avgPoint.y);
